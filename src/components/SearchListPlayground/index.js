...
class SearchListPlayground extends Component {
    render() {
        ...
        const Tag = disabled ? 'span' : 'label';

        return(
            <Tag className={classNameBlock}>
                <input value={value} type='checkbox' className="b-search-list-playground__input" onChange={onChange} id={id} defaultChecked={checked}/>
                {!disabled ? 
                ...
                {disabled ? 
                    <label htmlFor={id} className="b-search-list-playground__delete">
                        <img className="b-search-list-playground__delete-icon" src={deleteIcon} alt="Удалить" />
                    </label>
                    : null
                }
            </Tag>
